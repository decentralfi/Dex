...
export class RoundedValuePipe implements PipeTransform {
  ...
  transform(value: number): string {
    ...
    if(rounded.toString().length == 6){
      ...
      strValue = this.decimalPipe.transform(+Fixed1,'1.2');

    } else if(rounded.toString().length > 3 && rounded.toString().length <= 5){

      strValue = this.decimalPipe.transform(value,'1.0-2');
    ...
    } else if(rounded.toString().length > 12 && rounded.toString().length <= 15){
      sign = ' T';
      Fixed1 =  (rounded / trillion).toFixed(1);
      strValue = this.decimalPipe.transform(+Fixed1,'1.2');

    } else if(rounded.toString().length > 15 && rounded.toString().length <= 18){
      sign = ' Q';
      Fixed1 =  (rounded / quadrillion).toFixed(1);
      strValue = this.decimalPipe.transform(+Fixed1,'1.2');

    } else if(rounded.toString().length > 18 && rounded.toString().length <= 21){
      sign = ' Qt';
      Fixed1 =  (rounded / quintillion).toFixed(1);
      strValue = this.decimalPipe.transform(+Fixed1,'1.2');

    } else if((rounded.toString().length > 0 && rounded.toString().length <= 3) || rounded == 0){
      strValue = this.decimalPipe.transform(value,'1.2');
    }else{
