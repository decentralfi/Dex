...
import { VolumesTable, VolumesData, Table, Tables, PoolHistory, Chart, AssetPoints, Volumes } from '../interfaces/volumes';
...
export class VolumesService {
  ...
  public getHistory(field: string, period: string, network: string): Observable<any>{
    return this.http.get(this.endpointUrl + 'history/field/' + field + '/time/' + period + '/network/' + network + '/')
    .pipe(map((data: PoolHistory[]) => this.processHistoryData(data,network)));
  }

  processHistoryData(poolHistory: PoolHistory[],network: string):Chart{

    let enableStatus = 'enabled';
    let filtered = poolHistory.filter(pool => pool.asset.status == enableStatus);
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      if(filtered[i].field == 'slipaverage'){
        let arrayValue = filtered[i].value * 100;
        if(network == 'multichain_chaosnet'){
          arrayValue = filtered[i].value;
        }
        value = [moment(filtered[i].time).format('MM/DD/YYYY HH:[00]'), (arrayValue).toFixed(2)];
      }else if(filtered[i].field == 'swaps'){
        value = [moment(filtered[i].time).format('MM/DD/YYYY HH:[00]'),(filtered[i].value).toFixed(2)];
      }else{
        value = [moment(filtered[i].time).format('MM/DD/YYYY HH:[00]'),(filtered[i].value / 100000000).toFixed(2)];
      }
  ...
  public getVolumesTable(period: string, network: string){
    ...
    .pipe(map((data: VolumesData[]) => this.processVolumesTable(data,network)));
  }

  processVolumesTable(volumesData: VolumesData[],network: string): Tables{

    let enableStatus = 'enabled';
    let filtered = volumesData.filter(pool => pool.asset.status == enableStatus);
    if(network == 'multichain_chaosnet'){
      enableStatus = 'available';
      filtered = volumesData.filter(pool => pool.asset.status == enableStatus);
    }
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      let avgswapsizeRow = {rank: rank,asset: asset,value: avgswapsizeValue};
      let slipaverageRow = {rank: rank,asset: asset,value: slipaverageValue};
