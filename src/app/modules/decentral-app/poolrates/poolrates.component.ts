...
import { PoolRates, PoolRatesTable, Chart, AssetPoints, EndpointAsset, Totals } from '../shared/interfaces/poolrates';
...
const ELEMENT_DATA: PoolRatesTable[] = [
  {asset: { fullname: 'THOR.THOR', nameChain: 'THOR.THOR', chain: 'THOR', symbol: 'THOR', ticker:'THOR', iconPath: 'https://unpkg.com/cryptoicons-cdn@0.1.22/images/RUNE.png' },
  depth: 0,
  price: 0,
  roi: 0,
  slip: 0,
...
const CHART_COLORS = ['#22DBBF','#34C73B','#00A2FF','#EB5353','#FF9800','#FFEC3A','#8CC34A','#2096F3','#3F51B5','#673AB7'];
...
export class PoolratesComponent implements OnInit {
  ...
  public defaultAssetLogo: string =  'https://unpkg.com/cryptoicons-cdn/images/RUNE.png';
  ...
  public roiChart: any;
  public pools = [
    {asset: {chain: 'BNB', ticker:'RUNE', iconPath : 'https://unpkg.com/cryptoicons-cdn/images/RUNE.png'}, depth: 0, value: 0}
  ];
  ...
  public displayedColumns: string[] = ['pool', 'asset', 'price', 'depth', 'volume', 'swaps', 'roi', 'stakers', 'slip'];
  ...
  ngOnInit() {
    ...
    this.currencyService.getGlobalCurrency().subscribe(currency =>{
      ...
      this.poolratesService.getOriginalPoolRatesTable().subscribe((originalPoolratesTable: PoolRatesTable[]) => {
      ...
      this.poolratesService.getOriginalPoolRatesModule().subscribe(poolratesModule => {
        if(poolratesModule != null && this.poolRatesModule){
          let tempPoolratesModule = {};
          for (let i = 0; i < moduleFields.length; i++) {
            let poolrates;
            if(moduleFields[i].field == 'roi' || moduleFields[i].field == 'stakers'){
              let poolRatesTable = poolratesModule[moduleFields[i].field].table;
              let tempTable = [];
              ...
              let poolRatesChart = poolratesModule[moduleFields[i].field].chart.assetPoints;
              let poolRatesRawChart = poolratesModule[moduleFields[i].field].chart.rawChart;
              let chart = {};
              let assetPoints = [];
          ...
          //console.log(tempPoolratesModule);
          //console.log(this.poolRatesModule);
          this.poolRatesModule = tempPoolratesModule;
          this.pools = tempPoolratesModule[this.selectedParam].table;
          let selectedRow = this.poolratesTable.filter(selected => selected.asset.nameChain == this.selectedAsset);
  ...
  executeQuery(){
    ...
    /** POOLRATES TABLE */
    this.poolratesService.getPoolratesTable(this.perdiodRangeLabel,this.networkValue,this.currency).subscribe((data) => {
      this.poolratesTable = data;
      this.dataSource = new MatTableDataSource(this.poolratesTable);
      console.log('poolrates');
      this.setHideLoader();
    ...
    /**TOTALS */
    this.poolratesService.getTotals(this.perdiodRangeLabel,this.networkValue).subscribe((totals: Totals) => {

      for (let i = 0; i < moduleFields.length; i++) {

        this.poolRatesModule[moduleFields[i].field].total = totals[moduleFields[i].field];
        this.poolRatesModule[moduleFields[i].field].tooltip = totals['tooltip_' + moduleFields[i].field];
        console.log('totals');
        this.setHideLoader();
  ...
  processPoolsTables(data: PoolRatesTable[]){
    ...
    for (let i = 0; i < moduleFields.length; i++) {
      ...
      for (let x = 0; x < slicedData.length; x++) {
        ...
        let item = {rank: x+1, asset: slicedData[x].asset, depth: slicedData[x].depth, value: value};
        poolTable.push(item);
      }

      this.poolRatesModule[moduleFields[i].field].table = poolTable;
      if(moduleFields[i].field == 'roi'){
        console.log('roi tables');
        this.setHideLoader();
      }

      /**HISTORY */
      this.poolratesService.getHistory(moduleFields[i].field,this.perdiodRangeLabel, this.networkValue,slicedData).subscribe((data: Chart) => {
        this.poolRatesModule[moduleFields[i].field].chart = data;
        if(moduleFields[i].field == 'roi'){
          console.log('roi chart');
          this.setHideLoader();
        }
        ...
        if(moduleFields[i].field == 'roi' || moduleFields[i].field == 'stakers' || moduleFields[i].field == 'price'){
          priceCounter++;
          ...
          console.log('error loading ' + moduleFields[i].field + ' chart data: ' + error);
          this.setHideLoader();
      });
      /** */
  ...
  processDataChart(field, array, asset){
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if(field == 'roi'){
        value = [moment(array[i].time).format('MM/DD/YYYY HH:[00]'), (array[i].value * 100).toFixed(2)];
      }else if(field == 'stakers'){
        value = [moment(array[i].time).format('MM/DD/YYYY HH:[00]'),(array[i].value).toFixed(2)];
      }else{
        value = [moment(array[i].time).format('MM/DD/YYYY HH:[00]'),(array[i].value / 100000000).toFixed(2)];
      }
    ...
    if(this.lastUpdatedFlag == false){
      ...
      this.lastUpdatedLabel = moment(array[arrayLen - 1].time).format('MM/DD/YYYY HH:[00]');
  ...
  applyFilter(event: Event) {
    ...
    this.dataSource.filterPredicate = function(data, filter: string): boolean {
      return data.asset.ticker.toLowerCase().includes(filter) || data.asset.chain.toLowerCase().includes(filter);
    };

    let newfilteredData = [];

    for (let i = 0; i < this.poolratesTable.length; i++) {
      let predicate = this.dataSource.filterPredicate(this.poolratesTable[i],filterValue);
      if(predicate == true){
  ...
  public getPoolsCategory(){
    if(this.selectedPoolCategories.length == 0 || this.selectedPoolCategories.length == 3){
      this.selectedPoolCategories = ['Stablecoins','Cryptocurrencies','Tokens'];
      this.allLabel = 'All';
    }
...
setHideLoader(){
  ...
  console.log(this.loaderCounter+ ' == ' +this.loaderCounterLimit);
  if(this.loaderCounter == this.loaderCounterLimit){
    ...
    setTimeout(() => {
      ...
      console.log(this.poolRatesModule);
      console.log(this.poolratesTable);
    }, 500);
...
setPoolRatesCharts() {
  ...
  for (let i = 0; i < this.poolratesTable.length; i++) {
    ...
    let roiChart = this.setPoolRatesChartsByField('roi',this.poolratesTable[i],this.roiChart);
    let priceChart = this.setPoolRatesChartsByField('price',this.poolratesTable[i],this.priceChart);
    let stakersChart = this.setPoolRatesChartsByField('stakers',this.poolratesTable[i],this.stakersChart);
...
setPoolRatesChartsByField(label: string,poolratesRow: PoolRatesTable, fieldChart: EndpointAsset[]){
    ...
    for (let x = 0; x < fieldChart[label].length; x++) {
      ...
      if(chartassetName == poolName){
        for (let i = 0; i < fieldChart[label][x].pool.length; i++) {
          let time = moment(fieldChart[label][x].pool[i].time).format('MM/DD/YYYY HH:[00]');
          let value: any;
          if(label == 'roi'){ 
...
getPaginatorClass(size){
  ...
  setOptions() {
    ...
    this.options = {
      ...
      toolbox: {
        show: true,
        feature: {
          saveAsImage: {show: true, title: 'Save as Image', name: "decentralfiChart"}
        },
        iconStyle: {
  ...
  getPriceChartSeries(){
    ...
    let filter = raw.filter(pool=> this.poolratesService.getAssetName(pool.asset.name).nameChain == poolName);
    if(filter.length > 0){
      for (let y = 0; y < filter.length; y++) {
        
        let time = moment(filter[y].time).format('MM/DD/YYYY HH:[00]');
        let value: any;
        if(filter[y].field == 'roi'){ 
          value = (filter[y].value * 100).toFixed(2);
        }else if (filter[y].field == 'price'){
          value = filter[y].value.toFixed(4);
        }else{
  ...
  processPoolratesTable(array){
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      let getAssetName = findDash == -1 ? array[i].asset.name.substr(findDot + 1) : array[i].asset.name.substr(findDot + 1,findDash - 4);
  ...
  onChange($event) {
    ...
    this.selectedAsset = $event.asset.nameChain;
    this.setLastSectionOptions();
  }
  ...
  setLastSectionOptions() {
    ...
    this.assetPriceOptions = {
      ...
      series: [
        this.getPriceChartSeries(),
        this.getROIChartSeries()
      ]
  };
