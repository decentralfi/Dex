...
import { Network, NetworkTotal, NetworkPendulum, History } from '../shared/interfaces/network';
...
export class NetworkComponent implements OnInit {
  ...
  public incentivePendulum = [
    {status: 'Inefficient', class: 'inefficient', short: 'Further decrase node rewards', desc: 'Further decrase node rewards and further increase liquidity rewards'},
    {status: 'Over-Bonded', class: 'overbonded', short: 'Decrase node rewards', desc: 'Decrease node rewards and increase liquidity rewards'},
    {status: 'Optimal', class: 'optimal', short: 'None', desc: 'None'},
    {status: 'Under-Bonded', class: 'underbonded', short: 'Increase node rewards', desc: 'Increase node rewards and decrease liquidity rewards'},
    {status: 'UnSafe', class: 'unsafe', short: 'Further increase node rewards', desc: 'Further increase node rewards and further decrease liquidity rewards'},
  ];
  ...
  ngOnInit() {
    ...
    this.currencyService.getGlobalCurrency().subscribe(currency =>{
      ...
      this.networkService.getOriginalNetwork().subscribe((originalNetwork: Network) => {
        if(originalNetwork != null && this.networkModule){
          
          let networkModule = {};
          for (let i = 0; i < moduleFields.length; i++) {

            let filter = fields.filter(field => field == moduleFields[i].field);
            if(filter.length > 0){

              let total = originalNetwork[moduleFields[i].field].total / price;
              let tooltip = originalNetwork[moduleFields[i].field].tooltip;
              let chart = {assetPoints: []};

              for (let y = 0; y < originalNetwork[moduleFields[i].field].chart.assetPoints.length; y++) {
                let time = originalNetwork[moduleFields[i].field].chart.assetPoints[y][0];
                let value = originalNetwork[moduleFields[i].field].chart.assetPoints[y][1] / price;
              ...
              networkModule[moduleFields[i].field] = originalNetwork[moduleFields[i].field];
            }
  ...
  automaticQuery(){
    ...
    console.log('baseHour: ' + baseHour);
    console.log('actualHour: ' + actualHour.format('MM/DD/YYYY HH:mm'));
    console.log('nextHour: ' + nextHour.format('MM/DD/YYYY HH:mm'));
    console.log(this.duration.asMilliseconds());
  ...
  executeQuery(){
    ...
    for (let i = 0; i < moduleFields.length; i++) {
      ...
      this.networkService.getHistory(moduleFields[i].field,this.perdiodRangeLabel,this.networkValue,this.currency).subscribe((data: Network) => {
        this.networkModule[moduleFields[i].field] = data
        console.log(moduleFields[i].field);
        this.setHideLoader();
      });
  ...
  processDataChart(array, label?: string){
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if(label == 'totalactivenode' || label == 'totalstakers' || label == 'totalactiveusers' || label == 'totalblockheight'){
  ...
  setHideLoader(){
    ...
    if(this.loaderCounter == this.loaderCounterLimit){
      ...
      setTimeout(() => {
        ...
        this.initFlag = 0;
        console.log(this.networkModule);
  ...
  setOptions() {
    ...
    this.runeSupplyOptions = {
      ...
      legend: {
          ...
          data: ['Liquidity provider (15.31%)', 'Active nodes (43.88%)', 'Standby nodes (7.14%)', 'Standby pools (0.00%)', 'Outside of network (33.67%)']
      },
      series: [
          {
              name: 'SERIE',
