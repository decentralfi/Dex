...
import { Volumes, Table, AssetPoints, VolumeTotal, History, Totals, Chart } from '../shared/interfaces/volumes';
...
const ELEMENT_DATA: Table[] = [
  {rank: 1, asset: { fullname: 'THOR.THOR', nameChain: 'THOR.THOR', chain: 'THOR', symbol: 'THOR', ticker:'THOR', iconPath: 'https://unpkg.com/cryptoicons-cdn@0.1.22/images/RUNE.png' }, value: 0},
];
const CHART_COLORS = ['#FFEC3A','#8CC34A','#2096F3','#3F51B5','#673AB7','#9C27B0','#E91E63','#f34436','#22dbbf','#00edc9'];
...
export class VolumesComponent implements OnInit {
      ...
      this.volumesService.getOriginalVolumesModule().subscribe( volumes => {
        if( volumes != null){
          ...
          for (let i = 0; i < moduleFields.length; i++) {
            if(moduleFields[i].field != 'swaps' && moduleFields[i].field != 'slipaverage'){
              ...
              for (let x = 0; x < volumes[moduleFields[i].field].table.length; x++) {
                
                let value = +volumes[moduleFields[i].field].table[x].value / price;
              ...
              let assetPoint = {}
              for (let x = 0; x < volumes[moduleFields[i].field].chart.assetPoints.length; x++) {
                let points = [];
                for (let y = 0; y < volumes[moduleFields[i].field].chart.assetPoints[x].points.length; y++) {
                  let value = [volumes[moduleFields[i].field].chart.assetPoints[x].points[y][0],(+volumes[moduleFields[i].field].chart.assetPoints[x].points[y][1] / price).toFixed(2)];
                  points.push(value);
                }
                assetPoint = {
                  asset: volumes[moduleFields[i].field].chart.assetPoints[x].asset,
                  points: points
                };
              ...
              volumesModule[moduleFields[i].field] = volumes[moduleFields[i].field];
            }
          }

          this.volumesModule = volumesModule;

          this.dataSource = new MatTableDataSource(this.volumesModule[this.selectedParam].table);
  ...
  executeQuery(){
    ...
    for (let i = 0; i < moduleFields.length; i++) {
      ...
      this.volumesService.getHistory(moduleFields[i].field,this.perdiodRangeLabel,this.networkValue).subscribe(data => {
    ...
    /**TOTALS */
    this.volumesService.getTotals(this.perdiodRangeLabel,this.networkValue).subscribe((totals: Totals) => {

      for (let i = 0; i < moduleFields.length; i++) {

        this.volumesModule[moduleFields[i].field].total = totals[moduleFields[i].field];
        this.volumesModule[moduleFields[i].field].tooltip = totals['tooltip_' + moduleFields[i].field];
        console.log('totals');
        this.setHideLoader();
    ...
    this.volumesService.getVolumesTable(this.perdiodRangeLabel,this.networkValue).subscribe(data => {
      for (let i = 0; i < moduleFields.length; i++) {

        let ordered = this.orderTableByFieldDesc(data[moduleFields[i].field],'value');
  ...
  proccesDataByPool(field: string, data: any[]){
    ...
    for (let i = 0; i < arrayLenght; i++) {
      ...
      if(orderedArray[i].asset.status == 'enabled'){
        ...
        let getAssetName = findDash == -1 ? asset.substr(findDot + 1) : asset.substr(findDot + 1,findDash - 4);
        asset = getAssetName;
        ...
        chartArray.push(this.processDataChart(field,orderedArray[i].pool,getAssetName));
  ...
  getSeriesStep(){
    if(this.selectedParam == 'avgswapsize' || this.selectedParam == 'slipaverage'){
      return 'start';
    }else{
      return false
    }
  ...
  getChartSeries(){
    ...
    let series = [];
    let array: any[] = this.dataSource._pageData(this.volumesModule[this.selectedParam].chart.assetPoints);
  ...
  getChartLegend(){
    ...
    let array = this.dataSource._pageData(this.volumesModule[this.selectedParam].chart.labels);
    let newArray = [];
    for (let i = 0; i < array.length; i++) {
  ...
  processDataChart(field, array, asset){
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if(field == 'slipaverage'){
        ...
        value = [moment(array[i].time).format('MM/DD/YYYY HH:[00]'), (arrayValue).toFixed(2)];
      }else if(field == 'swaps'){
  ...
  setHideLoader(){
    ...
    if(this.loaderCounter == this.loaderCounterLimit){
      ...
      setTimeout(() => {
        ...
        this.initFlag = 0;
        console.log(this.volumesModule);
      }, 2000);
  ...
  orderCharts(){
    for (let i = 0; i < moduleFields.length; i++) {
      let labels: string[] = [];
      let chart: Chart;
      for (let x = 0; x < this.volumesModule[moduleFields[i].field].table.length; x++) {
        labels.push(this.volumesModule[moduleFields[i].field].table[x].asset.nameChain);
        
      }
      let assetPoints: AssetPoints[] = [];
      for (let z = 0; z < labels.length; z++) {
        let assetPointsFiltered = this.volumesModule[moduleFields[i].field].chart.assetPoints;
        let assetPointFiltered = assetPointsFiltered.filter(pool => pool.asset == labels[z]);
